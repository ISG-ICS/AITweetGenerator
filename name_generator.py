...
def gen_two_words(split=' ', lowercase=False):
    ...
    idx_first = np.random.randint(0, size)
    idx_last = np.random.randint(0, size)
...
def gen_one_word_digit(lowercase=False, digitmax=1000):
    size = len(ALL_ENG_NAMES) - 1
    idx_name = np.random.randint(0, size)
    digit = np.random.randint(0, int(digitmax))
...
def gen_year(startyear, endyear):
    return str(np.random.randint(int(startyear), int(endyear)))
